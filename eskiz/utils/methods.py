class Methods:
    ...
    SEND_INTERNATIONAL_SMS =  {'method': 'POST', 'path': 'message/sms/send-global'}
    GET_MESSAGE_DETAILS =     {'method': 'POST', 'path': 'message/sms/get-user-messages'}
    GET_MESSAGE_BY_DISPATCH = {'method': 'POST', 'path': 'message/sms/get-user-messages-by-dispatch'}
    GET_DISPATCH_STATUS =     {'method': 'POST', 'path': 'message/sms/get-dispatch-status'}
    ...
    @staticmethod
    def as_name(method_dict):
        for name, value in vars(Methods).items():
            if isinstance(value, dict) and value == method_dict:
                return name
